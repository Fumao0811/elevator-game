            ...
            <div className="screen-container" style={{ padding: 0, position: 'relative' }}>
                <div className={`door-container ${doorsOpening ? 'doors-opening' : ''} ${!isEscape ? 'walk-towards' : ''}`}>
                    {countdownNum <= 0 && pendingResultRoom && (
                        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundImage: `url(/door_empty.png)`, backgroundSize: 'cover', backgroundPosition: 'center', zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {isCaughtResult && otherPlayerInfo?.drawnImage && (
                                <img src={otherPlayerInfo.drawnImage} className="creep-forward" alt="scare" style={{ maxWidth: '300px', width: '80%', filter: 'drop-shadow(0 0 20px red)' }} />
                            )}
                        </div>
                    )}
    ...
    return (
        <div className="screen-container game-bg-anim" style={{ justifyContent: 'flex-start', paddingTop: '40px', backgroundImage: `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.85)), url(${bgImage})`, backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative' }}>
            ...
            <div style={{ margin: '20px 0' }}>
                ...
                <h2 className="highlight-role">{isEscape ? '逃げる側 (中)' : '待ち伏せ側 (外)'}</h2>
            ...
            <style>{`.disabled { pointer-events: none; } .floor-btn.forbidden { background: #222; color: #444; box-shadow: inset 0 0 10px #000; border: 1px solid #111; cursor: not-allowed; } .floor-btn.forbidden::after { border-color: transparent; } @keyframes pulse { 0% { opacity: 0.5; } 50% { opacity: 1; } 100% { opacity: 0.5; } } @keyframes fadeOutSplash { 0% { opacity: 1; } 70% { opacity: 1; } 100% { opacity: 0; pointer-events: none; } } @keyframes scaleUpSplash { 0% { transform: scale(0.5); opacity: 0; } 20% { transform: scale(1.1); opacity: 1; } 100% { transform: scale(1); opacity: 1; } }`}</style>
        </div>
    );
}
export default Game;
