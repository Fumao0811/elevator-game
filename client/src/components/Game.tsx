                ...
                <div className={`door-container ${doorsOpening ? 'doors-opening' : ''} ${!isEscape ? 'walk-towards' : ''}`}>
                            ...
                            {isEscape && isCaughtResult && otherPlayerInfo?.drawnImage && (
                                <div className="creep-forward-inside" style={{ position: 'relative', width: '300px', height: '400px', display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'visible' }}>
                                    <img
                                        src="/body.png"
                                        alt="scary body"
                                        style={{ position: 'absolute', bottom: '-40px', width: '200%', height: '200%', objectFit: 'contain', zIndex: 9998, filter: 'drop-shadow(0 0 10px red)', opacity: 1, display: 'block' }}
                                    />
                                    <img
                            ...
                            {!isEscape && isCaughtResult && otherPlayerInfo?.drawnImage && (
                                <div style={{ position: 'relative', width: '200px', height: '260px', animation: 'fx-flicker 0.5s infinite', overflow: 'visible' }}>
                                    <img
                                        ...
                                        style={{ position: 'absolute', bottom: '-20px', width: '200%', height: '200%', objectFit: 'contain', zIndex: 9998, opacity: 1, display: 'block' }}
                                    />
                                    <img
                                        src={otherPlayerInfo.drawnImage}
                                        alt="target face"
                    ...
                    <div style={{
                        ...
                        <h2 style={{
                            ...
                            padding: '10px 30px',
                            borderRadius: '50px',
                            color: isEscape ? '#4aff4a' : '#ff4a5a',
                            border: `2px solid ${isEscape ? '#4aff4a' : '#ff4a5a'}`,
                            boxShadow: `0 0 20px ${isEscape ? 'rgba(74,255,74,0.5)' : 'rgba(255,74,90,0.5)'}`,
                            fontSize: '1.5rem',
                            fontWeight: 'bold',
                            letterSpacing: '2px'
                        }}>
                            あなたは 【{isEscape ? 'エレベーターの中' : 'エレベーターの外'}】
                        </h2>
                    </div>
    ...
    return (
        ...
        >
            ...
            {showRoundSplash && (
                <div style={{
                    ...
                    <h1 style={{
                        fontSize: '3.5rem',
                        color: '#fff',
                        textShadow: '0 0 20px #ff2a3a',
                        letterSpacing: '5px',
                        animation: 'scaleUpSplash 1.5s cubic-bezier(0.2, 0.8, 0.2, 1) forwards'
                    }}>
                        Round {room.round}
                    </h1>
                    <h2 style={{
            ...
            <div style={{
                ...
                <div style={{ textAlign: 'center', color: '#aaa', fontSize: '0.9rem', marginBottom: '5px' }}>
                    Round {room.round} ({room.turn === 1 ? '表' : '裏'}) / {room.maxRounds}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-around' }}>
                    <div style={{ color: '#fff', fontSize: '0.9rem' }}>
                        あなたのポイント: <span style={{ color: '#ff2a3a', fontWeight: 'bold', fontSize: '1.2rem' }}>{myPlayerInfo?.score}</span>
                    </div>
                    <div style={{ color: '#aaa', fontSize: '0.9rem' }}>
                        相手のポイント: <span style={{ fontSize: '1.2rem' }}>{otherPlayerInfo?.score}</span>
                    </div>
                </div>
            </div>
            ...
            <div className={`elevator-board ${isWaiting ? 'disabled' : ''}`}>
                {floors.map((floor) => {
                    ...
                    return (
                        <button
                            key={floor}
                            className={`floor-btn ${selectedFloor === floor ? 'selected' : ''} ${isForbidden ? 'forbidden' : ''}`}
                            onClick={() => handleFloorSelect(floor)}
                            disabled={isWaiting || isForbidden}
                            style={{
                                opacity: isWaiting && selectedFloor !== floor ? 0.3 : 1,
                                filter: isForbidden ? 'grayscale(100%)' : 'none'
                            }}
                        >
                            {floor}F
                        </button>
            ...
            {room.history && room.history.length > 0 && (
                <div style={{
                    ...
                    <h3 style={{ fontSize: '1rem', color: '#aaa', marginBottom: '5px', textAlign: 'center', borderBottom: '1px solid #444', paddingBottom: '5px' }}>
                        📋 これまでの履歴
                    </h3>
                    {[...room.history].reverse().map((h: any, idx: number) => (
                        <div key={idx} style={{
                            fontSize: '0.95rem',
                            padding: '10px',
                            backgroundColor: h.isCaught ? 'rgba(255, 0, 0, 0.15)' : 'rgba(0, 255, 0, 0.1)',
                            borderLeft: `6px solid ${h.isCaught ? '#ff2a3a' : '#4aff4a'}`,
                            display: 'flex',
                            justifyContent: 'space-between',
                            color: '#eee',
                            fontWeight: 'bold'
                        }}>
                            <span>R{h.round}({h.turn === 1 ? '表' : '裏'})</span>
                            <span>
                                逃:{h.escapeNickname}({h.escapedFloor}F) 🆚 待:{h.waitNickname}({h.waitedFloor}F)
                            </span>
                            <span>{h.isCaught ? '❌捕獲' : '⭕逃走'}</span>
                        </div>
                    ))}
                </div>
            )}
