    ...
    return (
        <div className="screen-container game-bg-anim" style={{ justifyContent: 'flex-start', paddingTop: '40px', backgroundImage: `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.85)), url(${bgImage})`, backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative' }}>
            ...
            <div className={`elevator-board ${isWaiting ? 'disabled' : ''}`}>
                {floors.map((floor) => (
                    <button key={floor} className={`floor-btn ${selectedFloor === floor ? 'selected' : ''}`} onClick={() => handleFloorSelect(floor)} disabled={isWaiting} style={{ opacity: isWaiting && selectedFloor !== floor ? 0.3 : 1 }}>{floor}F</button>
                ))}
            </div>
            {isWaiting && (<div style={{ marginTop: 'auto', padding: '20px', color: '#fff', animation: 'pulse 1.5s infinite' }}>相手の選択を待っています...</div>)}
            <style>{`.disabled { pointer-events: none; } @keyframes pulse { 0% { opacity: 0.5; } 50% { opacity: 1; } 100% { opacity: 0.5; } }`}</style>
        </div>
    );
}
export default Game;
