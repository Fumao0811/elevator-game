...
function Result({ appState }: Props) {
    ...
    useEffect(() => {
        ...
        return () => { clearTimeout(timer); socket.off('start_next_round'); socket.off('game_finished'); };
    ...
    const handleNext = () => {
        if (isFinished) { appState.setRoom(null); navigate('/'); } 
        else { navigate('/game'); }
    };

    const handleShare = () => {
        const url = encodeURIComponent(window.location.origin);
        let text = isFinished ? `「エレベーター心理戦」をプレイしました！最終スコア: ${myPlayerInfo.score}pt\n${finalResultText}\n` : (isCaught ? `「エレベーター心理戦」で ${otherPlayerInfo.nickname} に捕まりました…👻\n` : `「エレベーター心理戦」で ${myPlayerInfo.selectedFloor}階へ逃げ切りました！\n`);
        window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${url}`, '_blank');
    };

    return (
        <div className="screen-container" style={{ backgroundColor: showResult ? '#1a1a1a' : '#fff', transition: 'background-color 0.1s' }}>
            {!showResult ? (
                isCaught && (
                ...
                <div className="result-content" style={{ animation: 'fadeIn 0.5s ease' }}>
                    ...
                    <div style={{ backgroundColor: '#222', padding: '20px', borderRadius: '12px', width: '100%', maxWidth: '300px', margin: '0 auto', border: '1px solid #444' }}>
                        <h3 style={{ color: '#888', marginBottom: '16px' }}>現在のスコア</h3>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.5rem' }}><span style={{ color: '#fff' }}>あなた</span><span style={{ color: '#ff2a3a', fontWeight: 'bold' }}>{myPlayerInfo.score} pt</span></div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '1.2rem', marginTop: '8px' }}><span style={{ color: '#aaa' }}>{otherPlayerInfo.nickname}</span><span style={{ color: '#aaa' }}>{otherPlayerInfo.score} pt</span></div>
                    </div>
                    ...
                    <div style={{ marginTop: '50px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
                        <button className="btn" onClick={handleNext} style={{ width: '250px' }}>{isFinished ? 'タイトルへ戻る' : '次のラウンドへ'}</button>
                        {isFinished && (<button className="btn" onClick={handleShare} style={{ width: '250px', backgroundColor: '#1DA1F2', borderColor: '#1DA1F2', color: '#fff' }}><span style={{ marginRight: '8px' }}>𝕏</span> 結果をポストする</button>)}
                    </div>
                </div>
            )}
            <style>{`@keyframes fadeIn { from { opacity: 0; transform: scale(0.95); } to { opacity: 1; transform: scale(1); } } @keyframes shake { 10%, 90% { transform: translate3d(-5px, 0, 0); } 20%, 80% { transform: translate3d(5px, 0, 0); } 30%, 50%, 70% { transform: translate3d(-10px, -10px, 0); } 40%, 60% { transform: translate3d(10px, 10px, 0); } } @keyframes flashBg { 0% { background-color: #000; } 50% { background-color: #300; } 100% { background-color: #000; } }`}</style>
        </div>
    );
}
